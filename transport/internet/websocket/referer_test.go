...
func TestDecodeWebSocketProtocolEarlyData(t *testing.T) {
	earlyData := []byte("hello websocket protocol")
	encoded := base64.RawURLEncoding.EncodeToString(earlyData)

	decoded, err := decodeWebSocketProtocolEarlyData(encoded)
	if err != nil {
		t.Fatalf("decodeWebSocketProtocolEarlyData failed: %v", err)
	}
	if !bytes.Equal(decoded, earlyData) {
		t.Fatalf("unexpected decoded early data: got %q want %q", decoded, earlyData)
	}
